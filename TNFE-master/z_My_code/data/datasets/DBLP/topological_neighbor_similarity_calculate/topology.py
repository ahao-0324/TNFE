...
def edge_weight_func(features_array):
    edge_weight_mat = np.zeros(shape=(len(features_array), len(features_array)), dtype=np.float32)
    for i in range(features_array.shape[0] - 1):
        print(i)
        for j in range(i + 1, features_array.shape[0]):
            sources = features_array[i, :]  # 取对应i节点的特征向量
            targets = features_array[j, :]  # 取对应j节点的特征向量
            intersection = np.logical_and(sources, targets)  # 对两个向量做与运算
            union = np.logical_or(sources, targets)  # 对两个向量做或运算
    ...
    np.save(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\DBLP\topological_neighbor_similarity_calculate\generated_data\edge_weight_mat.npy', edge_weight_mat)
    return edge_weight_mat

# def edge_weight_func2(features_list):
#     # edge_weight_mat = np.zeros(shape=(features_array.shape[0], features_array.shape[0]), dtype=np.float32)
#     # similarity_g = nx.Graph()
#     with open(
#             r'D:\Program Files\PycharmProjects\pythonProject\NSHE-master\z_My_code\data\datasets\APS\pre_data\rwr\node_similarity.txt',
#             'wb') as fl:
#         for i in range(len(features_list) - 1):
#             print(i)
#             for j in range(i + 1, len(features_list)):
#                 node_i = features_list[i][0]
#                 node_j = features_list[j][0]
#                 sources = np.array(features_list[i][1:])
#                 targets = np.array(features_list[j][1:])
#                 # intersection = np.logical_and(sources, targets)
#                 # union = np.logical_or(sources, targets)
#                 w = np.linalg.norm(sources - targets)  # 欧氏距离
#                 # tuple1 = (node_i, node_j, w)
#                 # tuple2 = (node_j, node_i, w)
#                 # similarity_list.append(tuple1)
#                 # similarity_list.append(tuple2)
#                 similarity_line1 = str(node_i) + "," + str(node_j) + "," + str(w) + "\n"
#                 similarity_line2 = str(node_j) + "," + str(node_i) + "," + str(w) + "\n"
#
#                 fl.write(similarity_line1.encode())
#                 fl.write(similarity_line2.encode())
#         fl.close()
...
def k_th_order_weighted_subgraph(G, w_adj_mat, k, node2id_dict):
    output = list()
    # G = nx.from_numpy_matrix(adj_mat)
    for node in nx.nodes(G):
        v_labels = [node2id_dict[name] for name, value in nx.single_source_shortest_path_length(G, node, cutoff=k).items()]  # 返回的列表，元素是以节点i为源节点，k为深度，的最短路径长度的节点名
        tmp_subgraph = w_adj_mat[np.ix_(v_labels, v_labels)]  # 输入节点最短路径列表，以列表中元素形成笛卡尔积映射，在w_adj_mat矩阵中定位元素形成新的矩阵，新的矩阵就是节点i的k阶邻居加权子图
...
# def k_th_order_weighted_subgraph2(G, k):
#     output = list()
#     tmp_list = list()
#     # G = nx.from_numpy_matrix(adj_mat)
#     # for i in range(G.number_of_nodes()):
#     #     v_labels = [name for name, value in nx.single_source_shortest_path_length(G, i, cutoff=k).items()]     # 返回的是最短路径邻居集合列表，元素是以节点i为源节点，k为深度，的最短路径长度的节点名
#     #     tmp_subgraph = w_adj_mat[np.ix_(v_labels, v_labels)]     # 输入节点最短路径名列表，以列表中元素形成笛卡尔积映射，在w_adj_mat矩阵中定位元素形成新的矩阵，新的矩阵就是节点i的k阶邻居加权子图
#
#         #tmp_subgraph = np.where(tmp_subgraph == 1., 1e-3, 1.-tmp_subgraph)
#         #np.fill_diagonal(tmp_subgraph, 0.)
#
#     filePath = r'D:\Program Files\PycharmProjects\pythonProject\NSHE-master\z_My_code\data\datasets\APS\pre_data\rwr\node_similarity.txt'
#     f2 = open(filePath,'rb')
#     idx = 1
#     for node in nx.nodes(G):
#         print('current line %s/%s' % (idx, nx.number_of_nodes(G)))
#         v_labels = [name for name, value in nx.single_source_shortest_path_length(G, node, cutoff=k).items()]   # 最短路径节点名列表
#         print("开始读取文件")
#         while 1:
#             lines = f2.readlines(100000)
#             if not lines:
#                 break
#             for line in lines:
#                 w_adj_mat = line.decode().split(",")
#                 # print(w_adj_mat)
#                 if ((w_adj_mat[0] in v_labels) or (w_adj_mat[1] in v_labels)):
#                     tmp_list.append(w_adj_mat)
#         # for i in range(len(w_adj_mat)):
#         #     if((w_adj_mat[i][0] in v_labels) or (w_adj_mat[i][1] in v_labels)):
#         #         tmp_list.append(w_adj_mat[i])
#         tmp_matrix = np.zeros(shape=(len(v_labels), len(v_labels)), dtype=np.float32)
#         print("创建新的k跳邻居加权矩阵")
#         for x in range(len(v_labels)-1):
#             for y in range(x+1, len(v_labels)):
#                 for z in range(len(tmp_list)):
#                     if((tmp_list[z][0] == v_labels[x]) and (tmp_list[z][1] == v_labels[y])):
#                         tmp_matrix[x][y] = tmp_list[z][2]
#                         tmp_matrix[y][x] = tmp_list[z][2]
#
#         output.append(tmp_matrix)         # 这里需要注意，图G中的每个节点的k邻居加权子图添加进output中，并不是以节点id对应，是以0,1,2...等数字对应的，所以在用的时候需要用G中节点id和数字匹配
#         idx = idx + 1
#     return output
