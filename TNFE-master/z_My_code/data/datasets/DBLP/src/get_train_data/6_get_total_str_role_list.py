...
with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\DBLP\pre_data\rwr\generated_data\het_neigh_train.txt','rb') as edge_file:
    neigh_lines = edge_file.readlines()
    for line in neigh_lines:
        neigh_list = line.decode().split()
        node_list.append(neigh_list[0])    # 将het_neigh_train.txt文件中没行的第一个节点保存在列表中
edge_file.close()
...
np.save(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\DBLP\train_data\total_str_role_list.npy', total_str_role_list)