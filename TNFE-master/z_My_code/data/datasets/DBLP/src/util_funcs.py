...
def shell_init(gpu_id=1, server='S5'):
    ...
    SOURCE_PATH = r'D:\Program Files\PycharmProjects\pythonProject\NSHE-master\src'
...
def load_nodes(dataset):
    ...
    in_path = r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\DBLP\train_data\node2id.txt'
    ...
    node2id = {}
    id2node = {}
    for i, line in enumerate(nodes):     # enumerate可以同时列出数据和数据下标
        token = line.strip('\n').split('\t')    # strip用于删除左右两侧的特殊字符，\t是制表符，表示四个空格（tab）
        if i == 0:
            node_cnt_all = int(token[0])
        elif i == 1:
            cur_type = token[0][0]      # 当前节点类型，取a,p,c
            type_info[cur_type] = {}
            type_info[cur_type]['start_id'] = int(token[1])     # 取节点对应的整数id，则type_info表示为{'a':[{'start_id':0},{'start_id':1},...],'p':[{},...],'c':[{},...]}
            node_id[cur_type] = {}
            node_id[cur_type][token[0]] = int(token[1])     # node_id表示为{'a':[{'a5988':0},{'a6793':1},...],'p':[{},...],'c':[{},...]}
        else:
    ...
    t_info = {}      # t_info表示为{'a': {'cnt': 1, 'ind': range(start, end)}}
    for t in node_id.keys():
        node_cnt[t] = len(node_id[t])
        type_info[t]['end_id'] = type_info[t]['start_id'] + node_cnt[t]
        t_info[t] = {}
        t_info[t]['cnt'] = node_cnt[t]      # cnt相当于count，表示某类型节点的数量
        t_info[t]['ind'] = range(type_info[t]['start_id'], type_info[t]['end_id'])     # ind相当于，表示节点该类型的节点，他们的id所在的范围
    return node_id, t_info, node2id, id2node


def load_relations(in_path, dataset="dblp", node_cnt=11576):
    # ============ Generate adj_mx ============
    with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\DBLP\train_data\topo_relaiton.txt') as edge_file:
    ...
    # ============ 获得rwr邻居列表，即局部网络结构 ============
    with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\DBLP\train_data\relation_neigh_structrole.txt','rb') as edge_file:
        neigh_lines = edge_file.readlines()

    row2, col2 = [], []
    for line in neigh_lines:
        token = line.decode().strip('\n').split('\t')
        row2.append(int(token[0]))
        col2.append(int(token[1]))
    _ = row2.copy()
    row2 = row2 + col2
    col2 = col2 + _
    edge2 = {"r": row2, "c": col2}
    ...
    return adj_torch, edge, len(row), adj, edge2
