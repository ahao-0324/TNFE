...
wasserstein_matrix = np.load(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\IMDB\topological_neighbor_similarity_calculate\generated_data\imdb_power_filtration_wasserstein_distances.npy')
...
# 进行边的权重的改变
# 设置增加边权重和减少边权重的阈值
added_threshold_down = 0.0    # 0.00070710684有两百八十多万个
added_threshold_up = 0.00071
removed_threshold = 0.11   # 0.11101577有7608个
idx = 0
        ...
        if (weight != 999 and w_type != 999):
            ...
            if (added_threshold_down < wasserstein_weight < added_threshold_up):
                ...
                if g.get_edge_data(source, target) != None:
                    # 更新边权
                    g[source][target][w_type]['w'] = weight + wasserstein_weight*1000
                # 两节点没有边，则添加边
                else:
...
with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\IMDB\pre_data\rwr\topology_induced_graph.pkl', 'wb') as f:
    pickle.dump(g, f)
