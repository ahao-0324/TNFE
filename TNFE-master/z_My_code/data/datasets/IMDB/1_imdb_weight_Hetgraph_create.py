...
# p_list = []
# v_list = []
with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\IMDB\train_data\relations.txt', 'r') as f2:
    for line in f2:
        line = line.strip()
        if (line.split('\t')[2] == '0'):
            m_node = id2node_dict[line.split('\t')[0]]
            a_node = id2node_dict[line.split('\t')[1]]
            g.add_edge(a_node, m_node, key=a_relation[0], w=1)
            g.add_edge(m_node, a_node, key=m_relation[0], w=1)
        if (line.split('\t')[2] == '1'):
            m_node = id2node_dict[line.split('\t')[0]]
            d_node = id2node_dict[line.split('\t')[1]]
            g.add_edge(m_node, d_node, key=m_relation[1], w=1)
            g.add_edge(d_node, m_node, key=d_relation[0], w=1)
...
with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\IMDB\pre_data\rwr\imdb_graph.pkl', 'wb') as f:
    pickle.dump(g, f)
