...
class Evaluation:
    def __init__(self, dataset='imdb'):
        ...
        if dataset == 'imdb':
            # read labels
            self.labels = []
            with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\IMDB\train_data\imdb_m_label.txt', 'r') as l_file:
    ...
    def evaluate_acm(self, emb_tensor):
        def _load_labels():
            x_a, l_a = [], []
            # Read Paper Labels
            with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\APS\pre_data\train_data\p_label.txt', 'r') as l_file:
                l_lines = l_file.readlines()
            for line in l_lines:
