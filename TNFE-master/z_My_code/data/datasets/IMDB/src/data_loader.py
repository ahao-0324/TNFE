...
def load_nodes(in_path, dataset="dblp"):    # 加载节点，将dblp数据集中所有节点分成a，p，c三类，以字典形式返回，比如'a':{里面是'a'对应的所有节点及其id}
    # Where to use ?
    print('Loading {} dataset...'.format(dataset))
    # ============ Generate adj_mx ============
    with open("{}/node2id.txt".format(in_path, dataset)) as node_file:
...
def load_relations(in_path, dataset="dblp", node_cnt=37682):
    ...
    for line in edges:
        ...
        row.append(token[0])
        col.append(token[1])
    adj = sp.csr_matrix((np.ones(len(row)), (row, col)), shape=(node_cnt, node_cnt))    # csr_matrix用来压缩稀疏矩阵
    # build symmetric adjacency matrix
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)  # 非对称邻接矩阵转变为对称邻接矩阵(有向图转无向图)
