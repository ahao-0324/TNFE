class Hyperparams:
    ...
    init_dw_emb = False  # Otherwise False   True表示直接加载,False表示重新生成
    norm_emb_flag = True
    dw_feat_only = False
    # ! Model config
    ...
    def __init__(self, dataset, task='cla'):
        ...
        optim_alpha = {'aps': {'cla': 0.001, 'clu': 0.001},        # cla表示分类任务，clu表示聚类任务，对于不同的任务这些参数有着不同的取值，所以需要分别训练
                       'dblp': {'cla': 0.008, 'clu': 0.008},
                       'imdb': {'cla': 0.008, 'clu': 0.008}}

        # 用于控制网络模式和成对接近度的比例
        optim_beta = {'aps': {'cla': 0.135, 'clu': 33.115},
                      'dblp': {'cla': 0.251, 'clu': 0.905},
                      'imdb': {'cla': 0.05, 'clu': 0.05}}

        optim_epoch = {'aps': {'cla': 1200, 'clu': 300},
