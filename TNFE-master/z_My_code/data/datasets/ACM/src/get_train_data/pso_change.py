...
class package:
    ...
    def __init__(self, neigh_num, target_node, neigh_list, index):
        import numpy as np
        # 最大权重
        self.Max_weight = 100
        # 物品数量
        ...
        # 当前节点的索引，用于在总的拓扑势列表中定位
        self.index = index

    # 根据0/1列表在邻居列表中筛选需要的列表
    def select_neigh_list(self, x, list):
        target_list = []
    ...
    # 计算拓扑势
    def compute_structural_role(self, G, node_i):
        ...
        for node_j in nx.nodes(G):
            if (node_j != node_i):
                ...
                # 获得网络中节点间最短路径长度
                if(node_j in shortest_path_length_dict.keys()):
                    dk = shortest_path_length_dict[node_j]
                else:
                    dk = 1
                # 最短加权路径长度(边的权重之和)
                if(node_j in shortest_path_length_weighted_dict.keys()):
    ...
    # 适应度函数
    def fit_func(self, x):    # 传入的x是列表，里面由0和1组成，表示所选择的节点
        import numpy as np
        import networkx as nx
        import pickle
        # 加载总图
        with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\rwr\ACM_graph.pkl','rb') as fr:
            G = pickle.load(fr)
        # 加载总的拓扑势列表
        total_str_role_list = np.load(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\train_data\total_str_role_list.npy',allow_pickle=True)
        ...
        # 根据0/1列表在邻居列表中筛选需要的列表
        target_list = self.select_neigh_list(x, self.neigh_list)
        target_list.append(self.target_node)
        # 计算损失函数
        local_str_role = self.compute_structural_role(nx.subgraph(G, target_list), self.target_node)
        total_str_role = total_str_role_list[self.index]
        loss = -abs(total_str_role - local_str_role)
        return loss
