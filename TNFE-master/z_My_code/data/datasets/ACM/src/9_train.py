...
def run(model, g, optimizer):
    for epoch in range(hp.epochs):
        ...
        loss = pairwise_loss + cla_loss * (np.exp(hp.beta))
        # loss = cla_loss * hp.beta
        loss.backward()
...
def main(hp):
    ...
    # 保存得到的嵌入
    torch.save(node_emb, r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\model_data\acm\final_embedding.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default='acm', help="dataset to train")
