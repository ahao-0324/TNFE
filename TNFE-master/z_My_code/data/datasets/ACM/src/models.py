...
class NSHE(nn.Module):
    ...
    def forward(self, adj, features, nsi_list):
        ...
        com_emb = self.neig_aggregator(encoded, adj)     # 返回的是Tensor类型
        if self.conv_method[-1] == '2':
        ...
        if self.cla_method == 'TypeSpecCla':
            ...
            context_emb = torch.FloatTensor(torch.zeros(com_emb.shape[0], self.context_dim)).to(device)
            ns_y = torch.FloatTensor([-1] * len(nsi_list)).to(device)
            for t in self.types:
