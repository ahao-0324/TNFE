...
wasserstein_matrix = np.load(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\topological_neighbor_similarity_calculate\generated_data\ACM_power_filtration_wasserstein_distances.npy')
...
# 进行边的权重的改变
# 设置增加边权重和减少边权重的阈值
added_threshold_down = 0.0025    # 等于0的有300万个,0.0028284274有800多万
added_threshold_up = 0.0029
removed_threshold = 0.109   # 小于0.109的有120个
idx = 0
        ...
        if (weight != 999 and w_type != 999):
            ...
            if (added_threshold_down < wasserstein_weight < added_threshold_up):
                ...
                if g.get_edge_data(source, target) != None:
                    # 更新边权
                    g[source][target][w_type]['w'] = weight + wasserstein_weight*100
                # 两节点没有边，则添加边
                else:
            ...
            if (wasserstein_weight > removed_threshold):
                new_weight = weight - wasserstein_weight*10
                # 更新后的边权重大于0，则更新边权
                if(new_weight > 0):
                    # 此时两节点有边，则减少边的权重
                    if g.get_edge_data(source, target) != None:
...
with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\rwr\topology_induced_graph.pkl', 'wb') as f:
    pickle.dump(g, f)
