...
with open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\train_data\node2id.txt', 'r') as f1:
    for line in islice(f1, 1, None):
        ...
        id = line.split('\t')[1]
        if(node[0] == 'p'):
            p_node2id_dict[node] = 'p' + id
            p_id2node_dict['p' + id] = node
        if(node[0] == 'a'):
            a_new_number_id = int(id) - 4019
            a_node2id_dict[node] = 'a' + str(a_new_number_id)
            a_id2node_dict['a' + str(a_new_number_id)] = node
        if (node[0] == 'v'):
            v_new_number_id = int(id) - 11186
            v_node2id_dict[node] = 'v' + str(v_new_number_id)
            v_id2node_dict['v' + str(v_new_number_id)] = node
...
a_neigh = np.load(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\rwr\generated_data\a_neigh.npy',allow_pickle=True)
a_neigh_w = np.load(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\rwr\generated_data\a_neigh_w.npy',allow_pickle=True)
p_neigh = np.load(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\rwr\generated_data\p_neigh.npy',allow_pickle=True)
p_neigh_w = np.load(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\rwr\generated_data\p_neigh_w.npy',allow_pickle=True)
...
neigh_f = open(r'D:\Program Files\PycharmProjects\pythonProject\mycode-master\z_My_code\data\datasets\ACM\pre_data\rwr\generated_data\het_neigh_train.txt', "w")
for i in range(2):
    for j in range(node_n[i]):
        if i == 0:
            neigh_train = a_neigh_list_train[j]
        ...
        if len(neigh_train):
            ...
            for k in range(len(neigh_train)-1):
                ...
                if (neigh_train[k][0] == "v"):
                    neigh_train[k] = v_id2node_dict[neigh_train[k]]
            ...
            if (neigh_train[-1][0] == "v"):
                neigh_train[-1] = v_id2node_dict[neigh_train[-1]]
